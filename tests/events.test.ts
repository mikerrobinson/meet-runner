...
import type { EntryLimits } from "../app/types/meet.ts";
...
/* --- course --- */
{
  ...
  eq(defaultEvents("m1", { course: "LCM" }).map(eventName).slice(14, 16), ["Girls 400 Free", "Boys 400 Free"], "defaultEvents follows the course");
  eq(defaultEvents("m1", { course: "SCY" }).map(eventName).slice(14, 16), ["Girls 500 Free", "Boys 500 Free"], "yards default unchanged");
  eq(defaultEvents("m1").length, 24, "no-argument default is the yards split lineup");
...
/* --- convert --- */
{
  const yards = defaultEvents("m1", { course: "SCY" });
  const metric = convertDistances(yards, "SCY", "LCM");

  // The converted yards lineup should read exactly like a native metric one.
  eq(metric.map(eventName), defaultEvents("m1", { course: "LCM" }).map(eventName), "converted lineup matches a native metric one");
  ...
  const long = [
    { id: "a", meetId: "m1", position: 0, distance: 500, stroke: "Free", gender: "Open" },
    { id: "b", meetId: "m1", position: 0, distance: 1000, stroke: "Free", gender: "Open" },
    { id: "c", meetId: "m1", position: 0, distance: 1650, stroke: "Free", gender: "Open" },
    { id: "d", meetId: "m1", position: 0, distance: 200, stroke: "IM", gender: "Open" },
    { id: "e", meetId: "m1", position: 0, distance: 400, stroke: "Free Relay", gender: "Open" },
    { id: "f", meetId: "m1", position: 0, distance: 1, stroke: "Diving", gender: "Open" },
  ] as const;
  ...
  eq(convertDistances([{ id: "g", meetId: "m1", position: 0, distance: 800, stroke: "Free", gender: "Open" }] as never, "SCY", "LCM").map((e) => e.distance), [800], "an 800 in a yards meet stays an 800");
  ...
  eq(convertDistances(defaultEvents("m1", { course: "SCY" }), "SCY", "LCM").filter((e) => e.distance === 400 && e.stroke === "Free").length, 2, "both genders of the distance free convert together");
...
/* --- diving --- */
{
  ...
  const split = defaultEvents("m1", { leadGender: "F", includeDiving: true });
  ...
  const boysFirst = defaultEvents("m1", { leadGender: "M", includeDiving: true });
  eq(boysFirst.slice(8, 10).map(eventName), ["Boys Diving", "Girls Diving"], "lead gender respected");

  const noDive = defaultEvents("m1", { leadGender: "F", includeDiving: false });
  ...
  eq(withDiving("m1", noDive, "F").map(eventName), split.map(eventName), "withDiving matches the standard order");
  eq(withDiving("m1", split, "F").length, 24, "adding twice is a no-op");
  eq(withoutDiving(split).map(eventName), noDive.map(eventName), "withoutDiving round-trips");

  // Open lineup gets a single Open event.
  const open = withDiving("m1", defaultEvents("m1", { mode: "open", includeDiving: false }), "F");
  eq(open.filter(isDiving).map(eventName), ["Diving"], "open lineup gets one Diving");

  // Empty lineup: gendered pair, appended.
  eq(withDiving("m1", [], "F").map(eventName), ["Girls Diving", "Boys Diving"], "empty lineup defaults to a pair");

  // A lineup with no 50 free puts diving at the end.
  const noFifty = defaultEvents("m1", { mode: "open", includeDiving: false }).filter((e) => e.distance !== 50);
  eq(withDiving("m1", noFifty, "F").at(-1)!.stroke, "Diving", "no 50 free -> appended");
...
// The rules live on the meet because states vary and a time trial wants none.
{
  ...
  const meet = (
    ...
    limits: EntryLimits = { maxIndividual: 2, maxRelays: 2, maxTotal: 4 },
  ) => ({ events, entries, limits });
  ...
  const withDiving = defaultEvents("m1", { course: "SCY", includeDiving: true });
